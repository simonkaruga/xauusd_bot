...
def _require_token(fn):
    """Decorator: reject mutating requests unless X-Dashboard-Token matches."""
    @functools.wraps(fn)
    def wrapper(*args, **kwargs):
        if not _DASHBOARD_TOKEN:
            # No token configured — block all mutating calls for safety
            return jsonify({'error': 'DASHBOARD_TOKEN env var not set'}), 403
        if request.headers.get('X-Dashboard-Token') != _DASHBOARD_TOKEN:
    ...
    <div class="card" style="padding:0;">
      <table>
        <thead><tr>
          ...
          <th>Volume</th><th>Unreal P&amp;L</th><th>Session</th><th>Regime</th><th>ML Score</th>
        </tr></thead>
        <tbody id="open-positions-body">
          <tr><td colspan="9" style="text-align:center;color:#8b949e;padding:24px;">No open positions</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
function refresh() {
  fetch('/api/data').then(function(r) { return r.json(); }).then(function(data) {
    ...
    var st = data.status;
    var pill = document.getElementById('bot-status');
    ...
    var dailyEl = document.getElementById('kpi-daily');
    dailyEl.textContent = '$' + fmt(data.daily_pnl);
    ...
    var opBody = document.getElementById('open-positions-body');
    if (data.open_trades && data.open_trades.length > 0) {
      opBody.innerHTML = data.open_trades.map(function(t) {
        var unreal = t[10] || 0;
        var unrealColor = unreal >= 0 ? '#3fb950' : '#f85149';
        var unrealSign = unreal >= 0 ? '+' : '';
        return '<tr>' +
          '<td><span class="badge badge-' + t[3].toLowerCase() + '">' + t[3] + '</span></td>' +
          '<td>' + fmt(t[5]) + '</td><td>' + fmt(t[7]) + '</td><td>' + fmt(t[8]) + '</td>' +
          '<td>' + fmt(t[9], 2) + '</td>' +
          '<td style="color:' + unrealColor + ';font-weight:600;">' + unrealSign + '$' + fmt(unreal) + '</td>' +
          '<td>' + (t[15] || '\u2014') + '</td><td>' + (t[16] || '\u2014') + '</td>' +
          '<td>' + fmt(t[19], 2) + '</td>' +
          '</tr>';
      }).join('');
    } else {
      opBody.innerHTML = '<tr><td colspan="9" style="text-align:center;color:#8b949e;padding:24px;">No open positions</td></tr>';
    }

    var tbody = document.getElementById('trades-body');
    if (data.trades && data.trades.length > 0) {
      tbody.innerHTML = data.trades.map(function(t) {
        var pnl = t[13] || t[10] || 0;
        var color = pnl >= 0 ? '#3fb950' : '#f85149';
        var sign = pnl >= 0 ? '+' : '';
        return '<tr>' +
          ...
          '<td>' + fmt(t[5]) + '</td>' +
          '<td>' + (t[6] ? fmt(t[6]) : '\u2014') + '</td>' +
          '<td style="color:' + color + ';font-weight:600;">' + sign + '$' + fmt(pnl) + '</td>' +
          '<td>' + (t[15] || '\u2014') + '</td>' +
          '<td>' + (t[16] || '\u2014') + '</td>' +
...
function drawEquityCurve(curve) {
  ...
  var maxV = Math.max.apply(null, vals.concat([0.01]));
  var range = maxV - minV || 1;
  var W = 800, H = 220, PAD = 10;
  function toX(i) { return PAD + (i / (vals.length - 1)) * (W - 2 * PAD); }
  function toY(v) { return PAD + (1 - (v - minV) / range) * (H - 2 * PAD); }

  var pts = vals.map(function(v, i) { return toX(i) + ',' + toY(v); });
  document.getElementById('equity-line').setAttribute('d', 'M ' + pts.join(' L '));
  document.getElementById('equity-fill').setAttribute('d',
    'M ' + toX(0) + ',' + toY(minV) + ' L ' + pts.join(' L ') +
...
function renderAttr(elemId, rows) {
  ...
  el.innerHTML = rows.map(function(r) {
    var wr = r[1] > 0 ? Math.round(r[2] / r[1] * 100) : 0;
    var pnl = r[3] || 0;
    ...
    return '<div class="attr-row">' +
      ...
      '<div style="text-align:right;">' +
        ...
        '<div style="color:' + pnlColor + ';font-size:13px;">' + sign + '$' + pnl + '</div>' +
      '</div>' +
    '</div>';
  }).join('');
}
